...
@Rule(
  key = CommonRulesConstants.RULE_INSUFFICIENT_OVERALL_BRANCH_COVERAGE,
  name = "Branches should have sufficient coverage by any tests",
  priority = Priority.MAJOR,
  tags = "bad-practice",
  description = "<p>An issue is created on a file as soon as the overall branch coverage on this file is less than the required threshold."
...
public class OverallBranchCoverageCheck extends CommonCheck {

  private static final double DEFAULT_RATIO = 65;

  @RuleProperty(key = CommonRulesConstants.PARAM_MIN_OVERALL_BRANCH_COVERAGE, description = "The minimum required branch coverage ratio.", defaultValue = "" + DEFAULT_RATIO)
  ...
  public void checkResource(Resource resource, DecoratorContext context, RuleKey rule, ResourcePerspectives perspectives) {
    double lineCoverage = MeasureUtils.getValue(context.getMeasure(CoreMetrics.OVERALL_BRANCH_COVERAGE), 0.0);
    if (context.getMeasure(CoreMetrics.OVERALL_BRANCH_COVERAGE) != null && lineCoverage < minimumBranchCoverageRatio) {
      double uncoveredConditions = MeasureUtils.getValue(context.getMeasure(CoreMetrics.OVERALL_UNCOVERED_CONDITIONS), 0.0);
      double conditionsToCover = MeasureUtils.getValue(context.getMeasure(CoreMetrics.OVERALL_CONDITIONS_TO_COVER), 0.0);
  ...
  private void createIssue(Resource resource, RuleKey ruleKey, double conditionsToCoverToReachThreshold, ResourcePerspectives perspectives) {
    createIssue(resource, perspectives, ruleKey, conditionsToCoverToReachThreshold, (int) conditionsToCoverToReachThreshold
      + " more branches need to be covered by tests to reach the minimum threshold of " + minimumBranchCoverageRatio
      + "% branch coverage.");
  }
